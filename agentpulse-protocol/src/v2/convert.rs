...
use agentpulse_core::{
    ...
    AgentMessageLevel, AgentMessageRole, AgentSession, AgentState, ApprovalCommandKind,
    ApprovalDisposition, ApprovalFileChange, ApprovalFileChangeKind, ApprovalNetworkContext,
    ApprovalOption, ApprovalRequest, ApprovalSelection, ApprovalSubject, ChannelCapabilities,
    ChannelDescriptor, ChannelKind, ChoiceOption, ChoiceOptionId, ChoiceRequest, ChoiceSelection,
    ConnectionState, DeterminateProgress, DomainError, EventSequence, ExternalId, FormAnswer,
    FormAnswerValue, FormField, FormFieldId, FormRequest, FormResponse, InteractionCloseReason,
...
use crate::{ProtocolError, ProtocolMessage, V2_PROTOCOL_VERSION};
...
pub(crate) fn decode_envelope(envelope: EnvelopeDto) -> Result<ProtocolMessage, ProtocolError> {
    if envelope.protocol_version != V2_PROTOCOL_VERSION {
...
fn encode_message(message: &AgentMessage) -> Result<AgentMessageDto, ProtocolError> {
    Ok(AgentMessageDto {
        role: encode_message_role(message.role())?,
...
fn decode_message(dto: AgentMessageDto) -> Result<AgentMessage, ProtocolError> {
    Ok(AgentMessage::with_role(
        decode_message_role(dto.role),
...
fn encode_interaction_request_payload(
    ...
    match payload {
        ...
        InteractionRequestPayload::Form(request) => Ok(InteractionRequestPayloadDto::Form {
            fields: request
                .fields()
                .iter()
                .map(|field| {
                    Ok(FormFieldDto {
                        ...
                        header: field.header().as_str().to_owned(),
                        prompt: field.prompt().as_str().to_owned(),
                        options: field
                            .options()
                            .iter()
                            .map(encode_choice_option)
                            .collect::<Result<_, ProtocolError>>()?,
                        allows_other: field.allows_other(),
...
fn decode_interaction_request_payload(
    ...
    match dto {
        ...
        InteractionRequestPayloadDto::Form { fields, blocking } => fields
            .into_iter()
            .map(|field| {
                FormField::new(
                    parse_id(field.id)?,
                    NonEmptyText::new(field.header)?,
                    NonEmptyText::new(field.prompt)?,
                    field
                        .options
                        .into_iter()
                        .map(decode_choice_option)
            ...
            .collect::<Result<Vec<_>, _>>()
            .and_then(|fields| FormRequest::new(fields, blocking).map_err(ProtocolError::from))
            .map(InteractionRequestPayload::Form),
    }
}
...
fn encode_interaction_response_payload(
    ...
    match payload {
        ...
        InteractionResponsePayload::Form(response) => Ok(InteractionResponsePayloadDto::Form {
            answers: response
                .answers()
                .iter()
                .map(|answer| FormAnswerDto {
                    field_id: answer.field_id().to_string(),
                    value: match answer.value() {
                        FormAnswerValue::Choice(option_id) => FormAnswerValueDto::Choice {
                            option_id: option_id.to_string(),
                        },
                        FormAnswerValue::Text(text) => FormAnswerValueDto::Text {
                            text: text.as_str().to_owned(),
                        },
                    },
                })
...
fn decode_interaction_response_payload(
    ...
    match dto {
        ...
        InteractionResponsePayloadDto::Text { text } => NonEmptyText::new(text)
            ...
            .map_err(ProtocolError::from),
        InteractionResponsePayloadDto::Form { answers } => answers
            .into_iter()
            .map(|answer| {
                let value = match answer.value {
                    FormAnswerValueDto::Choice { option_id } => {
                        FormAnswerValue::Choice(parse_id(option_id)?)
                    }
                    FormAnswerValueDto::Text { text } => {
                        FormAnswerValue::Text(NonEmptyText::new(text)?)
                    }
                };
                Ok(FormAnswer::new(
                    parse_id::<FormFieldId>(answer.field_id)?,
                    value,
                ))
            })
            .collect::<Result<Vec<_>, ProtocolError>>()
            .and_then(|answers| FormResponse::new(answers).map_err(ProtocolError::from))
            .map(InteractionResponsePayload::Form),
    }
}
...
fn encode_command_payload(
    ...
    match payload {
        AgentCommandPayload::SubmitPrompt { text, delivery } => {
            Ok(AgentCommandPayloadDto::SubmitPrompt {
                text: text.as_str().to_owned(),
                delivery: match delivery {
                    agentpulse_core::PromptDelivery::Queue => PromptDeliveryDto::Queue,
                    agentpulse_core::PromptDelivery::Steer => PromptDeliveryDto::Steer,
                },
            })
        }
        ...
        AgentCommandPayload::ListModels => Ok(AgentCommandPayloadDto::ListModels),
        AgentCommandPayload::SelectModel { model, effort } => {
            Ok(AgentCommandPayloadDto::SelectModel {
                model: model.to_string(),
                effort: effort.as_ref().map(ToString::to_string),
            })
        }
        AgentCommandPayload::SetPlanMode { enabled } => {
            Ok(AgentCommandPayloadDto::SetPlanMode { enabled: *enabled })
        }
        AgentCommandPayload::ListThreads { cursor } => Ok(AgentCommandPayloadDto::ListThreads {
            cursor: cursor.as_ref().map(ToString::to_string),
        }),
        AgentCommandPayload::ResumeThread { thread_id } => {
            Ok(AgentCommandPayloadDto::ResumeThread {
                thread_id: thread_id.to_string(),
            })
        }
        AgentCommandPayload::StartThread { cwd } => Ok(AgentCommandPayloadDto::StartThread {
            cwd: cwd.to_string(),
        }),
        AgentCommandPayload::Compact => Ok(AgentCommandPayloadDto::Compact),
        AgentCommandPayload::Review { instructions } => Ok(AgentCommandPayloadDto::Review {
            instructions: instructions.as_ref().map(ToString::to_string),
        }),
        AgentCommandPayload::Rename { name } => Ok(AgentCommandPayloadDto::Rename {
            name: name.to_string(),
        }),
        AgentCommandPayload::Fork => Ok(AgentCommandPayloadDto::Fork),
        AgentCommandPayload::Status => Ok(AgentCommandPayloadDto::Status),
        AgentCommandPayload::ListPermissionProfiles => {
            Ok(AgentCommandPayloadDto::ListPermissionProfiles)
        }
        AgentCommandPayload::SelectPermissionProfile { profile } => {
            Ok(AgentCommandPayloadDto::SelectPermissionProfile {
                profile: profile.to_string(),
            })
        }
        AgentCommandPayload::Queue { action } => Ok(AgentCommandPayloadDto::Queue {
            action: match action {
                agentpulse_core::QueueAction::Pause => QueueActionDto::Pause,
                agentpulse_core::QueueAction::Resume => QueueActionDto::Resume,
                agentpulse_core::QueueAction::Clear => QueueActionDto::Clear,
            },
        }),
...
fn decode_command_payload(
    ...
    match dto {
        AgentCommandPayloadDto::SubmitPrompt { text, delivery } => {
            Ok(AgentCommandPayload::SubmitPrompt {
                text: NonEmptyText::new(text)?,
                delivery: match delivery {
                    PromptDeliveryDto::Queue => agentpulse_core::PromptDelivery::Queue,
                    PromptDeliveryDto::Steer => agentpulse_core::PromptDelivery::Steer,
                },
            })
        }
        ...
        AgentCommandPayloadDto::ListModels => Ok(AgentCommandPayload::ListModels),
        AgentCommandPayloadDto::SelectModel { model, effort } => {
            Ok(AgentCommandPayload::SelectModel {
                model: NonEmptyText::new(model)?,
                effort: decode_optional_text(effort)?,
            })
        }
        AgentCommandPayloadDto::SetPlanMode { enabled } => {
            Ok(AgentCommandPayload::SetPlanMode { enabled })
        }
        AgentCommandPayloadDto::ListThreads { cursor } => Ok(AgentCommandPayload::ListThreads {
            cursor: decode_optional_text(cursor)?,
        }),
        AgentCommandPayloadDto::ResumeThread { thread_id } => {
            Ok(AgentCommandPayload::ResumeThread {
                thread_id: NonEmptyText::new(thread_id)?,
            })
        }
        AgentCommandPayloadDto::StartThread { cwd } => Ok(AgentCommandPayload::StartThread {
            cwd: NonEmptyText::new(cwd)?,
        }),
        AgentCommandPayloadDto::Compact => Ok(AgentCommandPayload::Compact),
        AgentCommandPayloadDto::Review { instructions } => Ok(AgentCommandPayload::Review {
            instructions: decode_optional_text(instructions)?,
        }),
        AgentCommandPayloadDto::Rename { name } => Ok(AgentCommandPayload::Rename {
            name: NonEmptyText::new(name)?,
        }),
        AgentCommandPayloadDto::Fork => Ok(AgentCommandPayload::Fork),
        AgentCommandPayloadDto::Status => Ok(AgentCommandPayload::Status),
        AgentCommandPayloadDto::ListPermissionProfiles => {
            Ok(AgentCommandPayload::ListPermissionProfiles)
        }
        AgentCommandPayloadDto::SelectPermissionProfile { profile } => {
            Ok(AgentCommandPayload::SelectPermissionProfile {
                profile: NonEmptyText::new(profile)?,
            })
        }
        AgentCommandPayloadDto::Queue { action } => Ok(AgentCommandPayload::Queue {
            action: match action {
                QueueActionDto::Pause => agentpulse_core::QueueAction::Pause,
                QueueActionDto::Resume => agentpulse_core::QueueAction::Resume,
                QueueActionDto::Clear => agentpulse_core::QueueAction::Clear,
            },
        }),
    }
}
...
fn decode_provider_capabilities(
    ...
    for value in values {
        ...
        capabilities |= match value {
            ...
            ProviderCapabilityDto::Control => ProviderCapabilities::CONTROL,
        };
    }
    Ok(capabilities)
}
...
fn encode_message_role(value: AgentMessageRole) -> Result<AgentMessageRoleDto, ProtocolError> {
    match value {
        AgentMessageRole::User => Ok(AgentMessageRoleDto::User),
        AgentMessageRole::Assistant => Ok(AgentMessageRoleDto::Assistant),
        AgentMessageRole::System => Ok(AgentMessageRoleDto::System),
        _ => Err(unsupported("AgentMessageRole")),
    }
}

const fn decode_message_role(value: AgentMessageRoleDto) -> AgentMessageRole {
    match value {
        AgentMessageRoleDto::User => AgentMessageRole::User,
        AgentMessageRoleDto::Assistant => AgentMessageRole::Assistant,
        AgentMessageRoleDto::System => AgentMessageRole::System,
    }
}
...
const fn provider_capability_name(value: ProviderCapabilityDto) -> &'static str {
    match value {
        ...
        ProviderCapabilityDto::Control => "control",
    }
}
