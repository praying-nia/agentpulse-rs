...
The first production Provider is [`agentpulse-provider-codex`](agentpulse-providers/codex). It manages a shared Unix-socket Codex App Server, strictly validates the byte-identical generated `0.152.0`/`0.152.1` schema for explicitly verified CLI versions, and starts valid newer SemVer releases best-effort with a visible warning while preserving strict protocol failure. It resumes explicit threads and publishes live read-only session events.

首个正式 Provider 是 [`agentpulse-provider-codex`](agentpulse-providers/codex)。它托管共享 Unix Socket Codex App Server，针对明确验证过的 CLI 版本严格校验 `0.152.0`/`0.152.1` 生成且逐字节相同的 Schema；更高的合法 SemVer 会在明确警告后尽力启动，但协议不兼容仍严格失败。Provider 恢复显式 Thread，并发布实时只读 Session Event。
...
The Rust workspace now powers a usable read-only product path: the `agentpulse` Host CLI owns a stable private identity and CA, explicit Codex threads, authenticated private-LAN Native WSS, mDNS discovery, QR-only public first pairing, per-device revocation, credential rotation, and an outbound Relay connector. `agentpulse-relay` allows disjoint stable Native and ephemeral pairing registrations, authenticates their routes over public TLS, and pumps the existing Host-CA TLS ciphertext without access to bootstrap/device Tokens or Session/Event plaintext. Session/Event and Relay route state remain in memory. Offline history, databases, and write-back remain separate future milestones. Production deployment assets are documented in [`deploy`](deploy).

Rust workspace 现已支撑可用的只读产品链路：`agentpulse` Host CLI 管理稳定私有身份与 CA、显式 Codex Thread、带认证私有 LAN Native WSS、mDNS 发现、纯二维码公网首次配对、逐设备撤销、凭证轮换及出站 Relay Connector。`agentpulse-relay` 允许互不重叠的稳定 Native 与临时配对注册共存，通过公网 TLS 认证路由，并在无法获取 Bootstrap/设备 Token 或 Session/Event 明文的前提下泵送既有 Host-CA TLS 密文。Session/Event 与 Relay 路由仍只保存在内存中；离线历史、数据库与写回属于后续独立里程碑。
